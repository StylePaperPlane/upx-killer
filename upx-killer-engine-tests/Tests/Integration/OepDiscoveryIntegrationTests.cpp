...
int RunOepDiscoveryIntegrationTests()
{
    ...
    auto const packedLength = GetEnvironmentVariableW(
        L"UPX_PACKED_FIXTURE", packedBuffer.data(), static_cast<DWORD>(packedBuffer.size()));
    ...
    Expect(result.outcome == EngineOutcome::Partial, "official UPX fixture is captured automatically");
    Expect(result.artifact && result.artifact->loaderMappable, "automatic artifact passes non-executing mapping");
...
int AnalyzeAutomaticOepTarget(std::filesystem::path const& target)
{
    ...
    std::wcout << L"candidate_count=" << (discovery.plan ? discovery.plan->candidates.size() : 0) << L'\n';
    if (discovery.plan)
    {
        for (auto const& candidate : discovery.plan->candidates)
        {
            std::wcout << L"transfer_rva=0x" << std::hex << candidate.transfer.value
                << L" target_rva=0x" << candidate.target.value << std::dec << L'\n';
        }
    }
...
int ValidateAutomaticOepTarget(std::filesystem::path const& target)
{
    ...
    return result.outcome == EngineOutcome::Partial || result.outcome == EngineOutcome::Completed ? 0 : 6;
}
...
int ValidateAutomaticOepTargetThroughHost(std::filesystem::path const& target)
{
    ...
    UnpackRequest request{};
    request.targetPath = target;
    request.outputPath = outputDirectory / (target.stem().wstring() + L".host.dumped.exe");
    request.timeoutMilliseconds = 60'000;
    ...
    return execution.protocolSucceeded &&
        (execution.result.outcome == EngineOutcome::Partial || execution.result.outcome == EngineOutcome::Completed) ? 0 : 6;
}
