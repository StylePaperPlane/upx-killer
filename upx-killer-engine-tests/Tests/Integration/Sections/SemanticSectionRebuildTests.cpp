...
namespace
{
    ...
    PeSection MakeSection(char const* name, std::uint32_t rva, std::uint32_t size, std::uint32_t characteristics)
    {
        PeSection section{};
    ...
    {
        auto const found = std::find_if(layout.sections.begin(), layout.sections.end(), [&](PeSection const& section) {
    ...
    PackedFixture MakePackedFixture()
    {
        ...
        fixture.layout.sections.push_back(MakeSection("m0dC0de", 0x1000, 0x3000,
            IMAGE_SCN_CNT_UNINITIALIZED_DATA | IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE | IMAGE_SCN_MEM_EXECUTE));
        fixture.layout.sections.push_back(MakeSection("xYz!42", 0x4000, 0x1000,
            IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE | IMAGE_SCN_MEM_EXECUTE));
        fixture.layout.sections.push_back(MakeSection(".rsrc", 0x5000, 0x1000,
            IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE));
        fixture.layout.directories[IMAGE_DIRECTORY_ENTRY_EXCEPTION] = { RelativeVirtualAddress{ 0x3000 }, sizeof(RUNTIME_FUNCTION) };
        fixture.layout.directories[IMAGE_DIRECTORY_ENTRY_RESOURCE] = { RelativeVirtualAddress{ 0x5000 }, 0x180 };
        ...
        auto* nt = reinterpret_cast<IMAGE_NT_HEADERS64*>(fixture.dump.bytes.data() + fixture.layout.ntHeaderOffset);
        ...
        nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION] = { 0x3000, sizeof(RUNTIME_FUNCTION) };
    ...
    void FixerAppliesSemanticSectionLayout()
    {
        ...
        auto const fixed = PeImageFixer::Rebuild(
            fixture.layout,
            fixture.dump,
        ...
        auto const hasPackedName = std::any_of(parsed.layout->sections.begin(), parsed.layout->sections.end(), [](auto const& section) {
            auto const name = NameOf(section);
            return name == "m0dC0de" || name == "xYz!42";
        });
        auto const hasRwx = std::any_of(parsed.layout->sections.begin(), parsed.layout->sections.end(), [](auto const& section) {
        ...
        Expect(exception && NameOf(*exception) == ".pdata", "final exception directory belongs to .pdata");
        ...
        Expect(std::any_of(parsed.layout->sections.begin(), parsed.layout->sections.end(), [](auto const& section) {
            return NameOf(section) == ".reloc";
