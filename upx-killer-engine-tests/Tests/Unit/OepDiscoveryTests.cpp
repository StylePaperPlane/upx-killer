...
pe::PeSection Section(char const* name, std::uint32_t virtualAddress, std::uint32_t virtualSize,
                      std::uint32_t rawOffset, std::uint32_t rawSize,
...
void WriteTailTransfer(UpxLikeImage& image, std::size_t offset, std::uint32_t targetRva) {
  ...
  auto const transferRva =
      image.layout.sections[1].virtualAddress.value +
...
int RunOepDiscoveryTests() {
  ...
  Expect(result.plan && result.plan->candidates.size() == 1,
         "discovery returns one bounded candidate");
  ...
  auto renamedResult =
      upx_killer::engine::pe::oep::UpxOepLocator::Analyze(renamed.bytes, renamed.layout);
  ...
  auto structuralResult =
      upx_killer::engine::pe::oep::UpxOepLocator::Analyze(renamed.bytes, renamed.layout);
  Expect(structuralResult.Succeeded(),
         "sparse executable destination supports marker-free UPX variant");

  UpxLikeImage outsideImage;
  WriteTailTransfer(outsideImage, 0x380, 0x5000);
  auto outsideResult =
      upx_killer::engine::pe::oep::UpxOepLocator::Analyze(outsideImage.bytes, outsideImage.layout);
  ...
  auto stubResult =
      upx_killer::engine::pe::oep::UpxOepLocator::Analyze(intoStub.bytes, intoStub.layout);
  ...
  auto truncatedResult =
      upx_killer::engine::pe::oep::UpxOepLocator::Analyze(truncated.bytes, truncated.layout);
  ...
  auto boundedResult =
      upx_killer::engine::pe::oep::UpxOepLocator::Analyze(bounded.bytes, bounded.layout);
