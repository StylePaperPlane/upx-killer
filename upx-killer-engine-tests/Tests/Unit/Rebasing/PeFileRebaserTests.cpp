...
int RunPeFileRebaserTests()
{
    ...
    if (rebased.image)
    {
        auto const* nt = reinterpret_cast<IMAGE_NT_HEADERS64 const*>(
            rebased.image->bytes.data() + parsed.layout->ntHeaderOffset);
        std::uint64_t relocated{};
        std::memcpy(&relocated, rebased.image->bytes.data() + 0x220, sizeof(relocated));
        expect(nt->OptionalHeader.ImageBase == 0x180000000ull, "rebased file records the required base");
        expect(relocated == 0x180001100ull, "DIR64 source slot receives the exact base delta");
        expect((nt->OptionalHeader.DllCharacteristics &
    ...
    auto rejected = pe::rebasing::PeFileRebaser::Rebase(
        malformed, *malformedLayout.layout, LoadedAddress{ 0x180000000ull });
