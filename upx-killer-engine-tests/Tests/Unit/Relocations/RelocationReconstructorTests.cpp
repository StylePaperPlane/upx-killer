...
int RunRelocationReconstructorTests() {
  ...
  layout.sizeOfHeaders = 0x200;
  layout.sizeOfImage = imageSize;
  auto rebuilt = pe::relocations::RelocationReconstructor::Reconstruct(
      snapshots, sourceSlots, layout, LoadedAddress{0x140000000ull});
  ...
  if (rebuilt.plan) {
    ...
    auto const* first =
        reinterpret_cast<IMAGE_BASE_RELOCATION const*>(rebuilt.plan->directoryBytes.data());
