...
namespace
{
    ...
    {
        auto const found = std::find_if(plan.sections.begin(), plan.sections.end(), [&](RebuiltSection const& section) {
            return rva >= section.virtualAddress.value && rva - section.virtualAddress.value < section.virtualSize;
        });
    ...
    void PackedSectionsBecomeAnalysisGradeSections()
    {
        ...
        source.sections.push_back(MakeSection("xYz!42", 0x4000, 0x1000,
            IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE | IMAGE_SCN_MEM_EXECUTE));
        source.sections.push_back(MakeSection(".rsrc", 0x5000, 0x1000,
            IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE));
        source.directories[IMAGE_DIRECTORY_ENTRY_EXCEPTION] = { RelativeVirtualAddress{ 0x3000 }, sizeof(RUNTIME_FUNCTION) };
        ...
        auto const result = SectionLayoutRebuilder::Build(
            source,
            { loaded, LoadedAddress{ 0x140000000ull }, RelativeVirtualAddress{ 0x1200 }, &imports });
        Expect(result.Succeeded(), "packed section layout is rebuilt");
        if (!result.plan) return;

        auto const hasPackedName = std::any_of(result.plan->sections.begin(), result.plan->sections.end(), [](auto const& section) {
            auto const name = NameOf(section);
            return name == "m0dC0de" || name == "xYz!42";
        });
        auto const hasRwx = std::any_of(result.plan->sections.begin(), result.plan->sections.end(), [](auto const& section) {
