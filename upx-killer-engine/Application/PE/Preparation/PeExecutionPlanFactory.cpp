...
using namespace upx_killer::engine::application::pe_preparation;

void FillDistinctBases(PeExecutionPlan& plan,
...
std::optional<PeExecutionPlan> PeExecutionPlanFactory::Create(
    pe::PeImageLayout const& image,
    PeBackendCapabilities const& capabilities) noexcept {
  if (!capabilities.Supports(image)) return std::nullopt;

  PeExecutionPlan plan{};
  plan.rebuildRelocations = image.sourceLoadPolicy.hasRelocations;
  plan.enableDynamicBase =
      plan.rebuildRelocations && image.sourceLoadPolicy.dynamicBase;
  plan.enableHighEntropyVa =
      plan.enableDynamicBase && image.format == pe::PeFormat::Pe64 &&
  ...
  auto const canonical = image.format == pe::PeFormat::Pe32
                             ? 0x00400000ull
                             : 0x140000000ull;
  plan.outputBase = LoadedAddress{
      plan.enableDynamicBase ? canonical : image.preferredImageBase};
  if (image.format == pe::PeFormat::Pe32) {
    if (image.imageKind == pe::PeImageKind::DynamicLibrary) {
      plan.captureBases = {LoadedAddress{0x10000000ull},
                           LoadedAddress{0x20000000ull},
                           LoadedAddress{0x28000000ull}};
      plan.captureCount = plan.captureBases.size();
    } else {
      FillDistinctBases(plan, {0x00400000ull, 0x10000000ull,
                               0x20000000ull, 0x28000000ull});
    }
    plan.validationBase = LoadedAddress{0x30000000ull};
  } else {
    FillDistinctBases(plan, {0x140000000ull, 0x180000000ull,
                             0x1c0000000ull, 0x200000000ull});
    plan.validationBase = LoadedAddress{0x240000000ull};
  }
  return plan.captureCount == 3 ? std::optional<PeExecutionPlan>{plan}
                                : std::nullopt;
}
}
