...
DumpResult ProcessImageDumper::Dump(IRemoteMemoryReader const& reader, LoadedImage loadedImage,
  ...
  try {
    ...
    while (offset < loadedImage.size) {
      ...
      auto destination =
          std::span<std::byte>{result.bytes}.subspan(static_cast<std::size_t>(offset), chunk);
