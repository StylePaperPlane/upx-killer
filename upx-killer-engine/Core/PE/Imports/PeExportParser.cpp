...
namespace
{
    ...
    {
        if (offset > bytes.size() || sizeof(T) > bytes.size() - static_cast<std::size_t>(offset)) return false;
        std::memcpy(&value, bytes.data() + static_cast<std::size_t>(offset), sizeof(T));
        return true;
    }

    bool RvaRange(std::uint32_t rva, std::uint64_t size, std::span<std::byte const> image, std::uint32_t& offset) noexcept
    {
...
{
    ExportParseResult PeExportParser::Parse(
        ...
        std::string moduleName) noexcept
    {
        try
        {
            ...
            if (!Read(image, 0, dos) || dos.e_magic != IMAGE_DOS_SIGNATURE || dos.e_lfanew < sizeof(IMAGE_DOS_HEADER))
                return { {}, ExportParseError::InvalidImage };
            auto const nt = static_cast<std::uint32_t>(dos.e_lfanew);
            DWORD signature{};
            IMAGE_FILE_HEADER file{};
            if (!Read(image, nt, signature) || !Read(image, nt + sizeof(signature), file) || signature != IMAGE_NT_SIGNATURE)
                return { {}, ExportParseError::InvalidImage };
            WORD magic{};
            auto const optional = static_cast<std::uint64_t>(nt) + sizeof(DWORD) + sizeof(IMAGE_FILE_HEADER);
            if (!Read(image, optional, magic)) return { {}, ExportParseError::Truncated };
            auto const sectionTable = optional + file.SizeOfOptionalHeader;
            auto const sectionBytes = static_cast<std::uint64_t>(file.NumberOfSections) * sizeof(IMAGE_SECTION_HEADER);
            if (file.NumberOfSections == 0 || file.NumberOfSections > 96 || sectionTable > image.size() || sectionBytes > image.size() - sectionTable)
                return { {}, ExportParseError::InvalidImage };
            std::vector<IMAGE_SECTION_HEADER> sections(file.NumberOfSections);
            for (WORD index = 0; index < file.NumberOfSections; ++index)
                if (!Read(image, sectionTable + index * sizeof(IMAGE_SECTION_HEADER), sections[index])) return { {}, ExportParseError::Truncated };
            std::uint32_t directoryRva{};
            std::uint32_t directorySize{};
            if (magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
            {
                IMAGE_OPTIONAL_HEADER64 header{};
                if (!Read(image, optional, header) || header.NumberOfRvaAndSizes <= IMAGE_DIRECTORY_ENTRY_EXPORT)
            ...
            else return { {}, ExportParseError::InvalidImage };
            if (directoryRva == 0 || directorySize < sizeof(IMAGE_EXPORT_DIRECTORY)) return { {}, ExportParseError::InvalidDirectory };
            std::uint32_t exportOffset{};
            if (!RvaRange(directoryRva, directorySize, image, exportOffset)) return { {}, ExportParseError::Truncated };
            IMAGE_EXPORT_DIRECTORY directory{};
            if (!Read(image, exportOffset, directory) || directory.NumberOfFunctions > MaxExports || directory.NumberOfNames > MaxExports)
                return { {}, ExportParseError::LimitExceeded };
            auto const functionBytes = static_cast<std::uint64_t>(directory.NumberOfFunctions) * sizeof(DWORD);
            ...
            {
                DWORD nameRva{}; WORD ordinalIndex{};
                if (!Read(image, static_cast<std::uint64_t>(directory.AddressOfNames) + index * sizeof(DWORD), nameRva) ||
                    !Read(image, static_cast<std::uint64_t>(directory.AddressOfNameOrdinals) + index * sizeof(WORD), ordinalIndex) ||
            ...
            {
                DWORD functionRva{};
                if (!Read(image, static_cast<std::uint64_t>(directory.AddressOfFunctions) + index * sizeof(DWORD), functionRva))
                    return { {}, ExportParseError::Truncated };
                ...
                else
                {
                    ...
                    exported.executable = std::any_of(sections.begin(), sections.end(), [&](IMAGE_SECTION_HEADER const& section) {
                        auto const extent = std::max(section.Misc.VirtualSize, section.SizeOfRawData);
                        return (section.Characteristics & IMAGE_SCN_MEM_EXECUTE) != 0 &&
                            functionRva >= section.VirtualAddress && functionRva - section.VirtualAddress < extent;
                    });
                }
        ...
        catch (...) { return { {}, ExportParseError::InvalidImage }; }
    }
}
