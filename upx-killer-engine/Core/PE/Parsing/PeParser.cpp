...
namespace upx_killer::engine::pe
{
    PeParseResult PeParser::Parse(std::span<std::byte const> bytes) noexcept
    {
        ...
        if (!Read(bytes, ntOffset, signature) || !Read(bytes, ntOffset + sizeof(signature), fileHeader))
        ...
        if (magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC) return { std::nullopt, PeError::UnsupportedPe32 };
        ...
        auto const sectionOffset = static_cast<std::uint64_t>(optionalOffset) + fileHeader.SizeOfOptionalHeader;
        auto const tableSize = static_cast<std::uint64_t>(fileHeader.NumberOfSections) * sizeof(IMAGE_SECTION_HEADER);
        ...
        auto const directoryCount = std::min<std::size_t>(optional.NumberOfRvaAndSizes, PeDirectoryCount);
        for (std::size_t i = 0; i < directoryCount; ++i)
        {
            layout.directories[i] = { { optional.DataDirectory[i].VirtualAddress }, optional.DataDirectory[i].Size };
        }
        ...
        for (std::uint16_t i = 0; i < fileHeader.NumberOfSections; ++i)
        {
            ...
            if (!Read(bytes, static_cast<std::size_t>(sectionOffset + i * sizeof(section)), section))
                return { std::nullopt, PeError::Truncated };
            auto const virtualExtent = std::max(section.Misc.VirtualSize, section.SizeOfRawData);
            if (!RangeFits(section.VirtualAddress, virtualExtent, optional.SizeOfImage) || section.VirtualAddress < previousEnd)
                return { std::nullopt, PeError::InvalidSectionTable };
            if (section.SizeOfRawData != 0 && !RangeFits(section.PointerToRawData, section.SizeOfRawData, bytes.size()))
                return { std::nullopt, PeError::Truncated };
