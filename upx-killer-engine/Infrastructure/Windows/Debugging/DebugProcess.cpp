...
DebugProcess::~DebugProcess() { Reset(); }
...
std::optional<DebugProcess> DebugProcess::Launch(std::filesystem::path const& applicationPath,
  ...
  auto const created =
      CreateProcessW(applicationPath.c_str(), commandLine.data(), nullptr, nullptr, FALSE,
  ...
  if (!SetInformationJobObject(result.job_, JobObjectExtendedLimitInformation, &limits,
                               sizeof(limits)) ||
...
void DebugProcess::Terminate(std::uint32_t exitCode) noexcept {
  if (job_)
    TerminateJobObject(job_, exitCode);
  else if (process_)
    TerminateProcess(process_, exitCode);
}
