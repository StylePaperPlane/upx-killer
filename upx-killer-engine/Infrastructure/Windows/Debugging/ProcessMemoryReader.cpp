...
bool IsWritable(DWORD protection) noexcept {
  if ((protection & (PAGE_GUARD | PAGE_NOACCESS)) != 0) return false;
  auto const access = protection & 0xffu;
...
dumping::MemoryRegion ProcessMemoryReader::Query(LoadedAddress address) const {
  ...
  if (VirtualQueryEx(process_, reinterpret_cast<void const*>(address.value), &information,
                     sizeof(information)) == 0)
    return {address, 0, false, false, false};
  return {{reinterpret_cast<std::uint64_t>(information.BaseAddress)},
          static_cast<std::uint64_t>(information.RegionSize),
          information.State == MEM_COMMIT && IsReadable(information.Protect),
          information.State == MEM_COMMIT && IsWritable(information.Protect),
          information.State == MEM_COMMIT && IsExecutable(information.Protect)};
}

std::size_t ProcessMemoryReader::Read(LoadedAddress address,
                                      std::span<std::byte> destination) const {
  SIZE_T read{};
  if (!ReadProcessMemory(process_, reinterpret_cast<void const*>(address.value), destination.data(),
