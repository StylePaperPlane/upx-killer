...
bool SoftwareBreakpointSet::Install(std::uint64_t address, std::uint32_t id,
                                    std::uint32_t& nativeError) noexcept {
  nativeError = ERROR_SUCCESS;
  if (Find(address)) return true;
  std::byte original{};
  SIZE_T read{};
  if (!ReadProcessMemory(process_, reinterpret_cast<void const*>(address), &original, 1, &read) ||
      read != 1) {
  ...
  SIZE_T written{};
  if (!WriteProcessMemory(process_, reinterpret_cast<void*>(address), &trap, 1, &written) ||
...
bool SoftwareBreakpointSet::Restore(std::uint64_t address, std::uint32_t& nativeError) noexcept {
  ...
  SIZE_T written{};
  if (!WriteProcessMemory(process_, reinterpret_cast<void*>(address), &iterator->original, 1,
                          &written) ||
      written != 1 || !FlushInstructionCache(process_, reinterpret_cast<void const*>(address), 1)) {
    nativeError = GetLastError();
    return false;
