...
StagedDebugTarget::~StagedDebugTarget() { Reset(); }

StagedDebugTarget::StagedDebugTarget(StagedDebugTarget&& other) noexcept
    : directory_(std::move(other.directory_)), executablePath_(std::move(other.executablePath_)) {
...
std::optional<StagedDebugTarget> StagedDebugTarget::Create(std::filesystem::path const& sourcePath,
                                                           pe::PeImageKind imageKind,
  ...
  try {
    ...
    auto const root = std::filesystem::temp_directory_path() / L"upx-killer" / L"debug-staging";
    ...
    for (unsigned attempt = 0; attempt < 32; ++attempt) {
      ...
      auto const name = L"stage-" + std::to_wstring(GetCurrentProcessId()) + L"-" +
                        std::to_wstring(GetTickCount64()) + L"-" + std::to_wstring(id);
    ...
    // A neutral staging name prevents Windows' compatibility database from
    // matching the original executable name and replacing IAT entries with
    // apphelp shim stubs before the OEP snapshot is captured.
    staged.executablePath_ = staged.directory_ /
        (imageKind == pe::PeImageKind::DynamicLibrary
             ? L"upx-killer-debug-target.dll"
             : L"upx-killer-debug-target.exe");
