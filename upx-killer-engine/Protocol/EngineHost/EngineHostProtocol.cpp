...
bool ReadFrameAny(HANDLE pipe, std::uint32_t& messageType, std::vector<std::byte>& payload) {
  ...
  if (!GetU32(header, offset, magic) || !GetU32(header, offset, version) ||
      !GetU32(header, offset, type) || !GetU32(header, offset, size))
    return false;
  if (magic != Magic || version != upx_killer::engine::protocol::ProtocolVersion ||
      size > upx_killer::engine::protocol::MaximumFrameSize)
    return false;
  messageType = type;
  payload.resize(size);
...
bool DecodeResult(std::span<std::byte const> payload, upx_killer::engine::EngineResult& result) {
  ...
  if (!GetU32(payload, offset, outcome) || !GetU32(payload, offset, error) ||
      !GetU32(payload, offset, native) || !GetU32(payload, offset, hasArtifact))
    return false;
  result = {};
...
bool WriteRequest(HANDLE pipe, UnpackRequest const& request) noexcept {
  try {
    ...
    PutU32(payload, request.retainFailedOutput ? 1u : 0u);
    PutU32(payload, request.imports ? 1u : 0u);
    PutU32(payload,
           request.imports ? static_cast<std::uint32_t>(request.imports->modules.size()) : 0u);
    if (request.imports) {
...
bool ReadRequest(HANDLE pipe, UnpackRequest& request) noexcept {
  try {
    ...
    std::uint32_t hasOep{}, oep{}, timeout{}, retainFailedOutput{}, hasImports{}, moduleCount{};
    ...
    if (!GetWide(payload, offset, target) || !GetWide(payload, offset, output) ||
        ...
        !GetU32(payload, offset, retainFailedOutput) || retainFailedOutput > 1 ||
        !GetU32(payload, offset, hasImports) || !GetU32(payload, offset, moduleCount) ||
        moduleCount > 4096)
      return false;
    request = {};
    ...
    request.retainFailedOutput = retainFailedOutput != 0;
    ...
    for (std::uint32_t moduleIndex = 0; moduleIndex < moduleCount; ++moduleIndex) {
      ...
      if (!GetNarrow(payload, offset, module.moduleName) ||
          !GetU32(payload, offset, module.firstThunk.value) ||
          !GetU32(payload, offset, symbolCount) || symbolCount > 65536)
        return false;
      for (std::uint32_t symbolIndex = 0; symbolIndex < symbolCount; ++symbolIndex) {
        ...
        if (!GetU32(payload, offset, byOrdinal) || !GetU32(payload, offset, ordinal) ||
            !GetU32(payload, offset, hint) || !GetNarrow(payload, offset, name))
          return false;
