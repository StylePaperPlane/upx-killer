...
namespace
{
    ...
    upx_killer::core::InspectionResult InspectPe(
        std::ifstream& stream,
        ...
        std::uint64_t fileSize) noexcept
    {
        ...
        if (!ReadAt(stream, fileSize, peOffset + coffHeader.size(), optionalMagicBytes.data(), optionalMagicBytes.size()))
        {
            return { std::nullopt, InspectionError::TruncatedFile };
        }
    ...
    upx_killer::core::InspectionResult InspectElf(
        std::ifstream& stream,
        std::filesystem::path const& path,
        std::uint64_t fileSize) noexcept
    {
...
namespace upx_killer::core
{
    InspectionResult TargetBinaryInspector::Inspect(std::filesystem::path const& path) noexcept
    {
        ...
        if (errorCode)
        {
            return {
                std::nullopt,
                errorCode == std::errc::permission_denied ? InspectionError::AccessDenied : InspectionError::IoFailure
            };
        }
