...
engine::EngineResult EngineHostClient::Execute(engine::UnpackRequest const& request,
                                               ProgressCallback const& progress) noexcept {
  try {
    if (!std::filesystem::is_regular_file(m_hostPath))
      return {engine::EngineOutcome::Failed, engine::EngineError::LaunchFailed, std::nullopt,
              ERROR_FILE_NOT_FOUND};
    if (!m_settingsStore) return {engine::EngineOutcome::Failed, engine::EngineError::LaunchFailed};
    ...
    if (temporaryFileSettings.deleteAfterExport) CleanupStaleArtifacts(artifactRoot);

    auto hostRequest = request;
    hostRequest.retainFailedOutput = !temporaryFileSettings.deleteAfterExport;
    ...
    UniqueHandle nullOutput{CreateFileW(L"NUL", GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    if (!CreateProcessW(
            ...
            EXTENDED_STARTUPINFO_PRESENT | CREATE_NO_WINDOW | CREATE_UNICODE_ENVIRONMENT, nullptr,
            m_hostPath.parent_path().c_str(), &startup.StartupInfo, &processInformation))
      return {engine::EngineOutcome::Failed, engine::EngineError::LaunchFailed, std::nullopt,
              GetLastError()};
    ...
    if (!engine::protocol::WriteRequest(requestWrite.get(), hostRequest)) {
      ...
      return {engine::EngineOutcome::Failed, engine::EngineError::ProtocolMismatch, std::nullopt,
              GetLastError()};
    }
    requestWrite.reset();
