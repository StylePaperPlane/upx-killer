...
NavigationPaneController::NavigationPaneController(
    ...
    : m_navigationView(navigationView), m_windowHandle(windowHandle) {
  if (!m_navigationView) {
  ...
  m_loadedToken = m_navigationView.Loaded({this, &NavigationPaneController::OnLoaded});
  m_unloadedToken = m_navigationView.Unloaded({this, &NavigationPaneController::OnUnloaded});
  m_paneOpeningToken =
  ...
  m_timerTickToken = m_timer.Tick({this, &NavigationPaneController::OnTimerTick});
}
...
void NavigationPaneController::OnPaneOpening(
  ...
  m_pointerObservedInsideOpenPane = IsPointerInsidePane(m_navigationView.OpenPaneLength());
}

void NavigationPaneController::OnPaneClosing(
    [[maybe_unused]] winrt::Microsoft::UI::Xaml::Controls::NavigationView const& sender,
    [[maybe_unused]] winrt::Microsoft::UI::Xaml::Controls::NavigationViewPaneClosingEventArgs const&
        args) {
...
void NavigationPaneController::OnTimerTick(
  ...
  bool const pointerInsideCompactPane = IsPointerInsidePane(m_navigationView.CompactPaneLength());
...
bool NavigationPaneController::IsPointerInsidePane(double paneWidthDips) const noexcept {
  ...
  if (!windowHandle || !GetCursorPos(&cursorPosition) ||
  ...
  auto const paneWidthPixels = static_cast<LONG>(paneWidthDips * scale + 0.5);

  return cursorPosition.x >= clientBounds.left && cursorPosition.x < paneWidthPixels &&
         cursorPosition.y >= clientBounds.top && cursorPosition.y < clientBounds.bottom;
}
