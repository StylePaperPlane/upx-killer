...
OverviewViewModel::OverviewViewModel() : m_resources() {
  ...
  m_startUnpackCommand = winrt::make_self<::upx_killer::ui::RelayCommand>(
      [this]() { StartUnpackAsync(); }, [this]() { return CanStart(); });
  m_exportCommand = winrt::make_self<::upx_killer::ui::RelayCommand>(
      [this]() { ExportAsync(); }, [this]() { return CanExport(); });
}
...
winrt::upx_killer::OverviewStatusKind OverviewViewModel::StatusKind() const noexcept {
  return m_statusKind;
}

bool OverviewViewModel::CanStart() const noexcept {
  return m_hasValidTarget && m_engineCompatible && !m_busy;
}
...
winrt::fire_and_forget OverviewViewModel::ExportAsync() {
  ...
  try {
    exported =
        m_artifactExporter && co_await m_artifactExporter->ExportAsync(m_windowId, m_outputPath);
  } catch (...) {
    exported = false;
  }
  m_busy = false;
  ...
  SetStatus(exported ? winrt::upx_killer::OverviewStatusKind::Succeeded
                     : winrt::upx_killer::OverviewStatusKind::Error,
...
void OverviewViewModel::ApplyInspectionResult(std::filesystem::path const& path,
...
void OverviewViewModel::SetStatus(winrt::upx_killer::OverviewStatusKind kind,
...
winrt::hstring OverviewViewModel::FormatFileSize(std::uint64_t bytes) const {
  ...
  if (bytes >= static_cast<std::uint64_t>(BytesPerMegabyte)) {
    stream << std::fixed << std::setprecision(2) << static_cast<double>(bytes) / BytesPerMegabyte
           << L" MB";
  } else if (bytes >= static_cast<std::uint64_t>(BytesPerKilobyte)) {
    stream << std::fixed << std::setprecision(2) << static_cast<double>(bytes) / BytesPerKilobyte
           << L" KB";
  } else {
...
winrt::hstring OverviewViewModel::FormatBinaryType(::upx_killer::core::BinaryFormat format) const {
  ...
  switch (format) {
    case BinaryFormat::Pe32Executable:
      return Resource(L"BinaryPe32Executable");
    case BinaryFormat::Pe32PlusExecutable:
      return Resource(L"BinaryPe32PlusExecutable");
    case BinaryFormat::Pe32Library:
      return Resource(L"BinaryPe32Library");
    case BinaryFormat::Pe32PlusLibrary:
      return Resource(L"BinaryPe32PlusLibrary");
    case BinaryFormat::Elf32Executable:
      return Resource(L"BinaryElf32Executable");
    case BinaryFormat::Elf64Executable:
      return Resource(L"BinaryElf64Executable");
    case BinaryFormat::Elf32SharedObject:
      return Resource(L"BinaryElf32SharedObject");
    case BinaryFormat::Elf64SharedObject:
      return Resource(L"BinaryElf64SharedObject");
  }

  return Resource(L"BinaryUnknown");
}

winrt::hstring OverviewViewModel::FormatArchitecture(
    ::upx_killer::core::BinaryArchitecture architecture) const {
  return architecture == ::upx_killer::core::BinaryArchitecture::X86 ? L"x86" : L"x64";
}

winrt::hstring OverviewViewModel::FormatInspectionError(
    ::upx_killer::core::InspectionError error) const {
  using ::upx_killer::core::InspectionError;
  switch (error) {
    case InspectionError::FileNotFound:
      return Resource(L"StatusFileNotFound");
    case InspectionError::AccessDenied:
      return Resource(L"StatusAccessDenied");
    case InspectionError::TruncatedFile:
      return Resource(L"StatusTruncatedFile");
    case InspectionError::UnsupportedFormat:
      return Resource(L"StatusUnsupportedFormat");
    case InspectionError::UnsupportedArchitecture:
      return Resource(L"StatusUnsupportedArchitecture");
    case InspectionError::IoFailure:
      return Resource(L"StatusIoError");
    case InspectionError::None:
      break;
  }
